...
class YOLOv10:
    ...
    def inference(self, image, input_tensor, conf_threshold=0.3):
        ...
        # === 检测框坐标平滑 ===
        if (
            self.last_boxes is not None
            and isinstance(boxes, np.ndarray)
            and len(boxes) == len(self.last_boxes)
            and len(boxes) > 0
        ):
            alpha = 0.4  # 平滑系数，可调
            boxes = self.last_boxes * (1 - alpha) + boxes * alpha
        self.last_boxes = boxes.copy() if isinstance(boxes, np.ndarray) else None
